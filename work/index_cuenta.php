	...
	<section class="full-box dashboard-contentPage">
		...
		<div class="container-fluid">
			<div class="page-header">
			  <h1 class="text-titles">Cuenta <small>Datos</small></h1>
			</div>
		</div>
		...
		<div class="modal-content">
			<form method="post" id="api_crud_form">
				<div class="modal-header">
		        	...
		        	<h4 class="modal-title">Datos Colegio</h4>
		      	</div>
		      	<div class="modal-body">
            ...
            <div class="col-md-6">
							...
							<select type="text" name="id_persona" class="form-control" id="id_persona" placeholder="Ingrese id_persona de colegio" maxlength=""></select>
	                       
	                        <br>
	                    </div>
	                    <div class="col-md-6">
							<label>Id Rol </label>
							<select type="text" name="id_rol" class="form-control" id="id_rol" placeholder="Ingrese id_rol" maxlength=""></select>
...
$(document).ready(function(){
	...
	$.ajax({
			 url: 'http://localhost/mi_control_colegio/api/test_api_usuario.php?action=fetch_all&pal=',
			 method: 'post',
			 dataType: 'json',
			 success: function (data) {
				 $(data).each(function (index, item) {
					id_persona.append($('<option/>', { value: item.id_persona, text: item.id_persona }));
				 });
			 },
			 error: function (err) {
		 ...
		 var id_rol = $('#id_rol');
	$.ajax({
			 url: 'http://localhost/mi_control_colegio/api/test_api_rol.php?action=fetch_all&pal=',
			 method: 'post',
			 dataType: 'json',
			 success: function (data) {
				 $(data).each(function (index, item) {
					id_rol.append($('<option/>', { value: item.id_rol, text: item.nombre }));
				 });
			 },
			 error: function (err) {
				 console.log(err.responseText);
	...
	$(document).on('click', '.edit', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('.modal-title').text('Modificar Datos Cuenta');
				$('#apicrudModal').modal('show');
			}
		})
	});
